...
from database_postgres import to_database
...
start_date = datetime(2024, 3, 26, 9, 0)
end_date = datetime(2024, 3, 26, 17, 0)
        ...
        for region in ["com"]:
            ...
            if region != "de":
                chrome_options.add_argument(
                    f'--load-extension=C:\\Users\\e.turchaninova\\PycharmProjects\\pythonProject1\\{extension}')
            # Теперь можно использовать driver для перехода на страницы с использованием указанного прокси
            ...
            while captcha_check(driver):
                print("Капча обнаружена. Попытка решить...")
            ...
            for keyword in key_words:
                ...
                for page in range(1, 21):
                    ...
                    try:
                        ...
                        print("Повторная загрузка страницы")
                        driver.get(url)
                        time.sleep(3)
                        matched_asins, all_asins, sh_dict = new_function(driver, region, matched_asins, all_asins, asin_codes, page,
                                                                date_for_excel, date_time, country, keyword, asin_to_sku,
